        ...
        for ((i0,j0),(x_bsz,y_bsz)),tn in plq.items():
            ...
            for i in range(i0,i0+x_bsz):
                for j in range(j0,j0+y_bsz):
                    if (i,j) in vx:
                        continue
                    vij = self.site_grad(tn.copy(),i,j) / cij 
                    if backend=='torch':
...
class Hamiltonian(ContractionEngine):
    ...
    def pair_energies_from_plq(self,config,peps,cache_bot,cache_top):
        ...
        for (site1,site2) in self.pairs:
            ...
            tn = plq.get(key,None) 
            if tn is not None:
                ex[site1,site2] = self.pair_energy_from_plq(tn.copy(),config,site1,site2) * sign
    ...
    def pair_energy_deterministic(self,config,peps,site1,site2,cache_bot,cache_top):
        ix1,ix2 = self.flatten(*site1),self.flatten(*site2)
        i1,i2 = config[ix1],config[ix2]
        if not self.pair_valid(i1,i2): # term vanishes 
            return 0. 
        ...
        for i1_new,i2_new,coeff in self.pair_terms(i1,i2):
            ...
            try:
                ...
                continue
        return eij * coeff_comm
    def pair_energies_deterministic(self,config,peps,cache_bot,cache_top):
        ...
        ex = dict()
        for (site1,site2) in self.pairs:
            ex[site1,site2] = self.pair_energy_deterministic(config,peps,site1,site2,cache_bot,cache_top) * sign
        return ex,cx,sign 
    def compute_local_energy(self,config,amplitude_factory,compute_v=True,compute_Hv=False):
        ...
        if self.deterministic:
            ex_dict,cx,sign = self.pair_energies_deterministic(config,peps,cache_bot,cache_top)
            err = 0.
        else:
            ex_dict,cx_dict,sign,plq = self.pair_energies_from_plq(config,peps,cache_bot,cache_top) 

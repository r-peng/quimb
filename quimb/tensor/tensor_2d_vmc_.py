...
class AmplitudeFactory(AmplitudeFactory_):
    ...
    def update_cache(self,config):
        ...
        self.cache_bot = cache_bot_new
        self.cache_top = cache_top_new
##################################################################################################
# contraction methods                                          
##################################################################################################
    ...
    def get_all_lenvs(self,cols,jmax=None,inplace=False):
        ...
        first_col = tmp.col_tag(0)
        lenvs = [None] * self.Ly
        for j in range(jmax+1): 
            tags = first_col if j==0 else (first_col,tmp.col_tag(j))
            try:
                tmp = self._contract_tags(tmp,tags)
    ...
    def get_all_renvs(self,cols,jmin=None,inplace=False):
        ...
        last_col = tmp.col_tag(self.Ly-1)
        renvs = [None] * self.Ly
        for j in range(self.Ly-1,jmin-1,-1): 
            tags = last_col if j==self.Ly-1 else (tmp.col_tag(j),last_col)
            try:
                tmp = self._contract_tags(tmp,tags)
                ...
                return cols,renvs
        return cols,renvs
    def _get_plq_forward(self,first_col,j,y_bsz,cols,renvs):
        # lenv from col, renv from renv
        tags = [cols.col_tag(j+ix) for ix in range(y_bsz)]
        ...
        plq.view_like_(cols)
        return plq
    def _get_plq_backward(self,last_col,j,y_bsz,cols,lenvs):
    ...
    def update_plq_from_3row(self,plq,cols,i,x_bsz,y_bsz,psi=None):
        jmax = self.Ly - y_bsz
        psi = self.psi if psi is None else psi
        cols,lenvs = self.get_all_lenvs(cols,jmax=jmax-1,inplace=False)
...
# for hamiltonian
################################################################################
    def _get_bot(self,imin,cache_bot=None):
        cache_bot = self.cache_bot is cache_bot is None else cache_bot
        bot = None if imin==0 else cache_bot[config[:imin*self.Ly]]
        return bot
    def _get_top(self,imax,cache_top=None):
        cache_top = self.cache_top is cache_top is None else cache_top
    ...
    def _get_boundary_mps_deterministic(self,config,imin,imax,cache_bot=None,cache_top=None):
        ...
        bot = self._get_bot(imin,cache_bot=cache_bot)
        top = self._get_top(imax,cache_top=cache_top)
        return bot,top
    def _unsigned_amplitude_deterministic(self,config,imin,imax,bot,top,cache_bot=None,cache_top=None):
        ...
        if top_term is None:
            return None
        return self._unsigned_amplitude_from_benvs(bot,top)
####################################################################################
###################################################################
# ham
###################################################################
from .tensor_vmc import Hamiltoian as Hamiltonain_
class Hamiltonian(Hamiltonian_):
    ...
    def _pair_energy_deterministic(self,config,site1,site2,top,bot):
        ...
        for i1_new,i2_new,coeff in self.model.pair_terms(i1,i2):
            ...
            cx_new = self.amplitude_factory._unsigned_amplitude_deterministic(config_new,
                         imin,imax,bot,top,cache_bot,cahce_top)
            if cx_new is None:
                continue
    ...
    def pair_energy_deterministic(self,config,site1,site2):
        ...
        bot,top = self.amplitude_factory._get_boundary_mps_deterministic(
                      config,imin,imax,cache_bot=chache_bot,cache_top=cache_top)
    ...
    def batch_pair_energies_deterministic(self,config,batch_imin,batch_imax):
        ...
        cache_bot = dict()
        bot,top = self.amplitude_factory._get_boundary_mps_deterministic(
                      config,batch_imin,batch_imax,cache_bot=cache_bot,cache_top=cahce_top)
        ex = dict() 
    ...
    def pair_energies_deterministic(self,config):
        self.amplitude_factory.get_all_benvs(config)
        ex = dict() 
        for (site1,site2) in self.model.pairs:
            imin = min(self.amplitude_factory.rix1+1,site1[0],site2[0]) 
            imax = max(self.amplitude_factory.rix2-1,site1[0],site2[0]) 
            bot = self.amplitude_factory._get_bot(imin)  
            top = self.amplitude_factory._get_top(imax)  
...
class ExchangeSampler(ExchangeSampler_):
    ...
    def _new_pair(self,site1,site2):
        ...
        if i1==i2: # continue
            return (None,) * 2
        i1_new,i2_new = self.propose_new_pair(i2,i1)
    ...
    def _update_pair(self,site1,site2,plq,cols):
        ...
        if config_sites is None:
            return plq,cols
        plq_new,py = self.amplitude_factory._new_prob_from_plq(plq,sites,confi_sites)
        if py is None:
            return plq,cols
        try:
        ...
        return plq_new,cols 
    def get_pairs(self,x_bsz,y_bsz):
        if (x_bsz,y_bsz)==(1,2):
    ...
    def update_plq(self,i,j,x_bsz,y_bsz,plq,cols):
        pairs = self.get_pairs(x_bsz,y_bsz)
        for site1,site2 in pairs:
            plq,cols = self._update_pair(site1,site2,plq,cols) 
        return cols
    def sweep_col_forward(self,i,cols,x_bsz,y_bsz):
        cols,renvs = self.amplitude_factory.get_all_renvs(cols,jmin=y_bsz,inplace=False)
        first_col = cols.col_tag(0)
        for j in range(self.Ly - y_bsz + 1): 
            plq = self.amplitude_factory._get_cols_forward(first_col,j,y_bsz,plq,renvs)
            cols = self.update_plq(i,j,x_bsz,y_bsz,plq,cols) 
            cols = self.amplitude_factory._contract_tags(cols,(first_col,cols.col_tag(j))
    def sweep_col_backward(self,i,cols,x_bsz,y_bsz):
        cols,lenvs = self.amplitude_factory.get_all_lenvs(cols,jmax=self.Ly-1-y_bsz,inplace=False)
        last_col = cols.col_tag(self.Ly-1)
        for j in range(self.Ly - y_bsz,-1,-1): # Ly-1,...,1
            plq = self.amplitude_factory._get_cols_backward(last_col,j,y_bsz,cols,lenvs)
            cols = self.update_plq(i,j,x_bsz,y_bsz,plq,cols) 
            cols = self.amplitude_factory._contract_tags(cols,(cols.col_tag(j+y_bsz-1),last_col))
    ...
    def update_plq_deterministic(self,i,j,x_bsz,y_bsz):
        ...
        for (site1,site2) in pairs:
            ...
            imin = min(site1[0],site2[0]) 
            imax = max(site1[0],site2[0]) 
            bot,top = self._amplitude_factory._get_boundary_mps_deterministic(self.config,imin,imax)
            cy = self._amplitude_factory._unsigned_amplitude_deterministic(config_new,imin,imax,bot,top)
            if cy is None:
                continue
    ...
    def _sample_deterministic(self):
        ...
        self.amplitude_factory.update_cache(self.config)

...
#    compress_opts['max_bond'] = None
#    compress_opts['dense'] = True
...
def compute_energy_term(term_key,benvs,directory,**compress_opts):
    ...
    compress_opts_ = compress_opts.copy()
#    compress_opts_['max_bond'] = None
#    compress_opts_['dense'] = True
    top_envs = ftn.compute_top_environments(yrange=(0,1),**compress_opts_)
...
def Hubbard(t,u,Lx,Ly,symmetry='u1',flat=True):
    ...
    return ham
def UEG(Nx,Ny,Lx,Ly,Ne,symmetry='u1',flat=True,maxdist=np.inf):
    # Nx,Ny: number of basis in each direction
    # Lx,Ly box length in each direction
    ...
    def get_pair(sites):
        ...
        if np.sqrt(dx**2+dy**2)<=maxdist:
            ...
            fac = 0.0
            for kn_dict in n_dict.values():
                fac += kn_dict['normsq']*kn_dict['cos']
            fac /= 2.0*len(n_dict) 
            # cre0,ann1 = (-1)**S ann1,cre0
            ops = cre_a.copy(),ann_a.copy()
            phase = (-1)**(cre_a.parity*ann_a.parity)
            ham[sites,'ke1a'] = ops,fac*phase
            # cre1,ann0
            ops = ann_a.copy(),cre_a.copy()
            phase = 1.0
            ham[sites,'ke2a'] = ops,fac*phase
            # cre0,ann1 = (-1)**S ann1,cre0 
            ops = cre_b.copy(),ann_b.copy()
            phase = (-1)**(cre_b.parity*ann_b.parity)
            ham[sites,'ke1b'] = ops,fac*phase
            # cre1,ann0
            ops = ann_b.copy(),cre_b.copy()
            phase = 1.0
            ham[sites,'ke2b'] = ops,fac*phase
            # e-e
            fac = 0.0
            for (nx,ny),kn_dict in n_dict.items():
                if not (nx==0 and ny==0):
                    fac += kn_dict['cos']/kn_dict['norm']
            fac *= 2.0*2.0*np.pi/(Lx*Ly)
    ...
    return ham
def _UEG(g,N,L,Ne,symmetry='u1',flat=True,maxdist=np.inf):
    from .block_interface import creation,onsite_U,ParticleNumber
    # parameters

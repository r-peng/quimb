from .fermion_core import FermionTensor, FermionTensorNetwork, FTNLinearOperator, _launch_fermion_expression
from .block_interface import set_options
from ..tensor_core import tensor_contract
from .fermion_2d import FPEPS, FermionTensorNetwork2D
from .block_gen import rand_all_blocks as rand
...
def run_simple_update(peps, ham, D=None, chi=None, tau_dict={0.1: 100, 0.01: 100}, energy_interval=np.inf, return_energy=True):
    ...
    for tau in taus:
        ...
        from pyblock3.algebra.fermion import TIMINGS
        timings = "td = %.3f qr = %.3f svd = %.3f tp = %.3f tot = %.3f" % (*TIMINGS, sum(TIMINGS))
        TIMINGS[0] = TIMINGS[1] = TIMINGS[2] = TIMINGS[3] = 0.0
        print('SU Sweep %5d / %5d NSTEP = %5d ENERGY = %20.6f Time = %10.3f %s' % (tau, len(taus), niters, Ei, tt, timings))
...
def get_u1_init_fermionic_peps(Nx, Ny, Ne, D, d, symmetry, random_particle_dist=False):
    ...
    print("Time for creating init peps = %10.3f" % (time.perf_counter() - tx))
...
#    n = ParticleNumber(symmetry=symmetry, flat=flat)
    ...
    op_dict = {'c_up':c_up, 'c_dn':c_dn, 'cd_up':cd_up, 'cd_dn': cd_dn, 'nund':nund}
    ...
    # Return results
    print("create hamil terms time = %10.3f" % (time.perf_counter() - tx))
...
def update_norm_left_boundary_environments(y, norm, benvs, chi=-1, 
                                           tmpdir=None, profile=False):
    """
    ...
    Args:
        ...
        norm: fname
            Column y-1 of the norm tensor network being contracted
        benvs: dict
    ...
    if y == 0:
        tn = write_ftn_to_disc(FermionTensorNetwork([]), tmpdir)
    ...
    # Do boundary contractions for remainder
        ...
        tn = load_ftn_from_disc(norm).copy()
        ls = [load_ftn_from_disc(benvs['left', y-1, 'norm']).copy(), tn]
        tn = FermionTensorNetwork(ls).view_as_(FermionTensorNetwork2D, like = tn)
        tn.contract_boundary_from_left_(xrange = (0, tn.Lx-1),
                                        yrange = (y-2, y-1),
                                        max_bond = chi, 
                                        layer_tags = ('KET','BRA'))
    ...
    # Return result
    next_key = 'left', y, 'norm'
    if profile:
        _profile(f'update_norm_left_boundary_{y}')
    return tn, next_key, []

def update_norm_right_boundary_environments(Ly, y, norm, benvs, chi=-1, 
                                            tmpdir=None, profile=False):
    """
    ...
    # Do boundary contractions for remainder
        ...
        tn = load_ftn_from_disc(norm).copy()
        ls = [tn, load_ftn_from_disc(benvs['right', y+1, 'norm']).copy()]
        tn = FermionTensorNetwork(ls).view_as_(FermionTensorNetwork2D, like = tn)
        tn.contract_boundary_from_right_(xrange = (0, tn.Lx-1),
                                         yrange = (y+1, y+2),
                                         max_bond = chi,
                                         layer_tags = ('KET','BRA'))
        tn = write_ftn_to_disc(tn, tmpdir)

    next_key = 'right', y, 'norm'
    # Return result
    if profile:
        _profile(f'update_norm_right_boundary_{y}')
    return tn, next_key, []

def update_ham_term_left_boundary_environments(Hi, y, norm, benvs, op_dict, 
                                               chi=-1,
                                               tmpdir=None,
                                               peps_parity=None,
                                               profile=False):
    ...
    if any(_y < y for _x, _y in Hi.sites):
        ...
        if not next_key in benvs:
            ...
            if y == 1:
                ...
                ls = [load_ftn_from_disc(benvs[prev_key]).copy(), norm.copy()]
                prev_tn = FermionTensorNetwork(ls).view_as_(FermionTensorNetwork2D, 
                                                            like = norm)
            ...
            # Return result
            if profile:
                _profile(f'update_ham_term_left_boundary_{y}')
            return prev_tn, next_key, applied_order

def update_ham_term_right_boundary_environments(Hi, Ly, y, norm, benvs, op_dict,
                                                chi=-1,
                                                tmpdir=None,
                                                peps_parity=None,
                                                profile=False):
    ...
    if any(_y > y  for _x, _y in Hi.sites):
        ...
        if not next_key in benvs:
            ...
            if y == norm.Ly-2:
                ...
                ls = [norm.copy(), load_ftn_from_disc(benvs[prev_key]).copy()]
                prev_tn = FermionTensorNetwork(ls).view_as_(FermionTensorNetwork2D, 
                                                            like = norm)
            ...
            # Return result
            if profile:
                _profile(f'update_ham_term_right_boundary_{y}')
            return prev_tn, next_key, applied_order

def update_term_left_boundary_environments(Hi, y, norm, benvs, op_dict,  
                                           chi=-1,
                                           tmpdir=None,
                                           peps_parity=None,
                                           profile=False):
    if Hi is None:
        return update_norm_left_boundary_environments(y, norm, benvs, 
                   chi=chi, tmpdir=tmpdir, profile=profile)
    elif y > 0:
        return update_ham_term_left_boundary_environments(Hi, y, norm, benvs, 
                   op_dict, chi=chi, tmpdir=tmpdir, peps_parity=peps_parity, 
                   profile=profile)

def update_term_right_boundary_environments(Hi, Ly, y, norm, benvs, op_dict,  
                                           chi=-1,
                                           tmpdir=None,
                                           peps_parity=None,
                                           profile=False):
    if Hi is None:
        return update_norm_right_boundary_environments(Ly, y, norm, benvs, 
                   chi=chi, tmpdir=tmpdir, profile=profile)
    elif y < Ly-1:
        return update_ham_term_right_boundary_environments(Hi, Ly, y, norm, benvs, 
                   op_dict, chi=chi, tmpdir=tmpdir, peps_parity=peps_parity,
                   profile=profile)
...
def update_left_boundary_environments(y, H, norm, benvs, 
    ...
    norm_prev = None if y == 0 else \
        write_ftn_to_disc(norm.select(norm.col_tag(y-1)).copy(), tmpdir)
    keys = [key for key in benvs.keys() if key[0] == 'left' and key[1] == y-1]
    benvs_tmp = {key:benvs[key] for key in keys}
    benvs_tmp['op_order'] = benvs['op_order']
    # Do boundary update term-by-term (parallelized)
    function = update_term_left_boundary_environments
    iterate_over = H.ham_terms + [None]
    args = [y,
            norm_prev,
            benvs_tmp, 
            H.op_dict]
    kwargs = {'chi':chi, 'peps_parity':peps_parity, 'tmpdir':tmpdir, 
              'profile': profile}
    results = parallelized_looped_function(function, iterate_over,
                                           args, kwargs)
    ...
    for i in range(len(results)):
        if results[i] is not None: 
            prev_tn = results[i][0]
            next_key = results[i][1]
            applied_order = results[i][2]
            benvs[next_key] = prev_tn
            benvs['op_order'][next_key] = applied_order
...
def update_right_boundary_environments(y, H, norm, benvs, 
    ...
    norm_prev = None if y == norm.Ly-1 else \
        write_ftn_to_disc(norm.select(norm.col_tag(y+1)).copy(), tmpdir)
    keys = [key for key in benvs.keys() if key[0] == 'right' and key[1] == y+1]
    benvs_tmp = {key:benvs[key] for key in keys}
    benvs_tmp['op_order'] = benvs['op_order']
    # Do boundary update term-by-term (parallelized)
    function = update_term_right_boundary_environments
    iterate_over = H.ham_terms + [None]
    args = [norm.Ly, y,
            norm_prev,
            benvs_tmp, 
...
def move_benvs_right(y, H, norm, benvs, chi=-1, replace_curr_col=True, peps_parity=None, tmpdir=None, profile=False):
...
#                    benvs[key]['tn'] = write_ftn_to_disc(new_tn, tmpdir)
    ...
    for key in rm_keys:
        ...
        if tmpdir is not None:
            os.remove(removed_env)

    # Return result
    return benvs

def move_benvs_left(y, H, norm, benvs, chi=-1, replace_curr_col=True, peps_parity=None, tmpdir=None, profile=False):
    """
...
def initialize_boundary_environments(y, H, norm, chi=-1, peps_parity=None, tmpdir=None, profile=False):
    """
    ...
    # Return the result
    print('time init envs = %10.3f' % (time.perf_counter() - tx))
    return norm, benvs
...
def get_left_env(x, y, H, benvs):
    ...
    # Get the left env tn
    left_tn = load_ftn_from_disc(benvs[left_key])

    return left_tn, left_key
...
def get_right_env(x, y, H, benvs):
    ...
    right_tn = load_ftn_from_disc(benvs[right_key])

    return right_tn, right_key
...
def get_column_and_env_tn(x, y, norm, H, benvs):
    ...
    Args:
        ...
        norm: FermionicTN
            Column y of the bra and ket peps combined into a TN
        H: OPTERM
            A single OPTERM
        benvs: dict
    ...
    left_env, left_key = get_left_env(x, y, H, benvs)
    right_env, right_key = get_right_env(x, y, H, benvs)
    ...
    tn = FermionTensorNetwork((left_env,
         ...
         FermionTensorNetwork2D, like=norm)
...
def update_norm_top_environments(Lx, x, y, norm, benvs, top_envs, chi=-1, 
                                 tmpdir=None, profile=False):
    """
    ...
    Args:
        ...
        y int
            The y position in the lattice
        norm: fname 
            Column y of the norm containing the bra nad ket peps
        benvs: dict
            The boundary environments
        bottom_envs: dict
    ...
    # First row - create a tn of the column and surrounding boundaries
        ...
        left_env = load_ftn_from_disc(benvs['left', y, 'norm'])
        right_env = load_ftn_from_disc(benvs['right', y, 'norm'])
        central_col = load_ftn_from_disc(norm)
        tn = FermionTensorNetwork((left_env, 
                                   central_col,
                                   right_env),
                                   check_collisions=False).view_as_(
             FermionTensorNetwork2D, like=central_col)

        # Do a reorder
        tn = tn.reorder("row", layer_tags=('KET', 'BRA'))

        # Save the result
        boundary_tag = tn.row_tag(Lx-1)
        tn = write_ftn_to_disc(tn, tmpdir)
    ...
    # Remainder of rows via boundary contraction
        ...
        tn.contract_boundary_from_top_(xrange = (x+1, x+2),
                                       yrange = (max(y-1, 0), min(y+1, tn.Ly-1)),
                                       max_bond = chi, 
                                       layer_tags = ('KET', 'BRA'))
        boundary_tag = tn.row_tag(Lx-1)
        tn = write_ftn_to_disc(tn, tmpdir)
    ...
    next_key = x, 'norm'
    if profile:
        _profile(f'update_norm_top_row_{x},{y}')
    return tn, next_key, boundary_tag, None

def update_norm_bottom_environments(x, y, norm, benvs, bottom_envs, chi=-1, 
                                    tmpdir=None, profile=False):
    """
    ...
    next_key = x, 'norm'
    if profile:
        _profile(f'update_norm_bottom_row_{x},{y}')
    return tn, next_key, boundary_tag, None

def update_ham_term_top_environments(Hi, Lx, x, y, norm, benvs, top_envs, op_dict,
                                     chi=-1, peps_parity=None,
                                     tmpdir=None, profile=False):
    """
    ...
    if x < Lx-1:
        ...
        # Return the results
        if profile:
            _profile(f'update_ham_term_top_row_{x},{y}')
        return tn, next_key, last_row, applied_order

def update_ham_term_bottom_environments(Hi, x, y, norm, benvs, bottom_envs, op_dict, 
                                        chi=-1, peps_parity=None,
                                        tmpdir=None, profile=False):
    """
    ...
    if x > 0:
        ...
        # Return the result
        if profile:
            _profile(f'update_ham_term_bottom_row_{x},{y}')
        return tn, next_key, first_row, applied_order

def update_term_top_environments(Hi, Lx, x, y, norm, benvs, top_envs, op_dict,
                                 chi=-1, peps_parity=None,
                                 tmpdir=None, profile=False):
    if Hi is None:
        return update_norm_top_environments(Lx, x, y, norm, benvs, top_envs, 
                   chi=chi, tmpdir=tmpdir, profile=profile)
    else: 
        return update_ham_term_top_environments(Hi, Lx, x, y, norm, benvs, top_envs, 
                   op_dict, chi=chi, peps_parity=peps_parity, tmpdir=tmpdir, 
                   profile=profile)

def update_term_bottom_environments(Hi, x, y, norm, benvs, bottom_envs, op_dict, 
                                    chi=-1, peps_parity=None,
                                    tmpdir=None, profile=False):
    if Hi is None:
        return update_norm_bottom_environments(x, y, norm, benvs, bottom_envs, 
                   chi=chi, tmpdir=tmpdir, profile=profile)
    else: 
        return update_ham_term_bottom_environments(Hi, x, y, norm, benvs, 
                   bottom_envs, op_dict, chi=chi, peps_parity=peps_parity, 
                   tmpdir=tmpdir, profile=profile)

#@profile
def update_top_environments(Lx, x, y, H, norm, benvs, top_envs, chi=-1, 
                            peps_parity=None, tmpdir=None, profile=False):
    """
    ...
    function = update_term_top_environments
    iterate_over = H.ham_terms + [None]
    args = [Lx, x, y, 
            norm,
            benvs,
...
#@profile
def update_bottom_environments(x, y, H, norm, benvs, bottom_envs, chi=-1, 
                               peps_parity=None, tmpdir=None, profile=False):
    """
    ...
    # Do the boundary update term-by-term (parallelized)
    function = update_term_bottom_environments
    iterate_over = H.ham_terms + [None]
    args = [x, y, 
            norm,
            benvs,
...
def initialize_top_environments(Lx, x, y, H, norm, benvs, chi=-1, 
                                peps_parity=None, tmpdir=None, 
                                profile=False):
    """
    ...
    for row in range(Lx-1, x-1, -1):
        benvs_tmp = benvs if row == Lx-2 else None
        top_envs = update_top_environments(Lx, row, y, H, norm, benvs_tmp, 
                       top_envs, chi=chi, peps_parity=peps_parity, 
                       tmpdir=tmpdir, profile=profile)

    # Return the result
    return top_envs

def initialize_bottom_environments(x, y, H, norm, benvs, chi=-1, 
                                   peps_parity=None, tmpdir=None, 
                                   profile=False):
    """
    ...
    for row in range(x+1):
        benvs_tmp = benvs if row == 1 else None
        bottom_envs = update_bottom_environments(row, y, H, norm, benvs_tmp, 
                          bottom_envs, chi=chi, peps_parity=peps_parity, 
                          tmpdir=tmpdir, profile=profile)

    # Return the result
    return bottom_envs

def move_top_environments_up(x, top_envs, tmpdir):
    """
...
def move_bottom_environments_up(x, y, H, norm, benvs, bottom_envs, 
                                replace_site=True, chi=-1, peps_parity=None, 
                                tmpdir=None, profile=False):
    """
    ...
    if replace_site:
        for key in bottom_envs:
            if type(key[0]) == int and key[0] > 0:
                old_tn = bottom_envs[key]['tn']
                new_tn = transfer_site_ten(x, y, norm, load_ftn_from_disc(old_tn))
                if tmpdir is not None:
                    os.remove(old_tn)
    ...
    if x < norm.Lx-1: 
        central_col = write_ftn_to_disc(norm.select(norm.col_tag(y)).copy(), tmpdir)
        bottom_envs = update_bottom_environments(x+1, y, H, central_col, benvs, 
                          bottom_envs, chi=chi, peps_parity=peps_parity, 
                          tmpdir=tmpdir, profile=profile)
        if tmpdir is not None:
            os.remove(central_col)
...
def local_ham_term(Hi, x, y, op_dict, 
                   top_envs, bottom_envs,
    ...
    # Contract
    if dense:
        Hi_tn = Hi_tn.contract(output_inds=bra_ten.inds[::-1]+ket_ten.inds)
    if profile:
        _profile(f'local_ham_term_{x},{y}')
...
def local_ham(x, y, H, norm, 
    ...
    bra_ten = norm[(f'I{x},{y}','BRA')].reindex_({f'k{x},{y}': f'b{x},{y}'})
    ket_ten = norm[(f'I{x},{y}','KET')]
    Hloc = FermionTensor(data=Hloc, inds=bra_ten.inds[::-1]+ket_ten.inds, 
                                    left_inds=bra_ten.inds[::-1])
    ...
    if linop:
        Hloc = FTNLinearOperator([Hloc,], 
                                 ...
                                 ket_ten.net_symmetry,
                                 constructor=constructor)
        bra_ten.reindex_({f'b{x},{y}': f'k{x},{y}'})
...
def calc_energy(peps, H, chi, write_to_disc=False, around=(0,0), symmetry=None):
    ...
    norm, benvs = initialize_boundary_environments(around[1], H, norm, 
                                                   ...
                                                   peps_parity=peps_parity, 
                                                   tmpdir=tmpdir)
    print('time 1 = %10.3f' % (time.perf_counter() - tx))

    # Contract around this row
    central_col = write_ftn_to_disc(norm.select(norm.col_tag(around[1])).copy(), 
                                    tmpdir)

    tx = time.perf_counter()
    top_envs = initialize_top_environments(norm.Lx, around[0], around[1], H, 
                                           central_col, benvs,
                                           peps_parity=peps_parity,
                                           tmpdir=tmpdir)
    print('time 2 = %10.3f' % (time.perf_counter() - tx))
    tx = time.perf_counter()
    bottom_envs = initialize_bottom_environments(around[0], around[1], H, 
                                                 central_col, benvs,
                                                 peps_parity=peps_parity,
                                                 tmpdir=tmpdir)
    print('time 3 = %10.3f' % (time.perf_counter() - tx))

    # Get into a local Norm/Hamiltonian
    ket0_ten, ket0_vec, Ni = local_norm(around[0], around[1], 
                                        top_envs, bottom_envs,
                                        peps_parity=peps_parity, 
                                        symmetry=symmetry)
    _, _, Hi = local_ham(around[0], around[1], H, norm,  
                         top_envs, bottom_envs,
...
def load_dynamical_parameters(tmpdir):
    """
    ...
    param_file = get_param_file(tmpdir)
...
def save_dynamical_parameters(tmpdir,eig_step_size=0.1, eig_maxiter=1000,
...
def dmrg_site_update(x, y, H, norm, top_envs, bottom_envs, 
                     profile=False,
    ...
    params = load_dynamical_parameters(tmpdir)
    ...
    ket0_ten, ket0_vec, Ni = local_norm(x, y, 
                                        top_envs, bottom_envs,
                                        peps_parity=peps_parity,
                                        symmetry=symmetry,
                                        profile=profile)
...
def dmrg_column_sweep(y, H, norm, benvs, peps, chi=-1, peps_parity=None, tmpdir=None, start_time=None, save_peps_loc=None, profile=False, **site_update_args):
    """
    ...
    x = 0
    central_col = write_ftn_to_disc(norm.select(norm.col_tag(y)).copy(), tmpdir)
    keys = [key for key in benvs.keys() if key[1] == y]
    benvs_tmp = {key:benvs[key] for key in keys}
    benvs_tmp['op_order'] = benvs['op_order']

    txx = time.perf_counter()
    print(f'\t\tInitializing top column environments ({time.time()-start_time} s)')
    top_envs = initialize_top_environments(norm.Lx, x, y, H, central_col, benvs_tmp, 
                   chi=chi, peps_parity=peps_parity, tmpdir=tmpdir, profile=profile)
    txx_tope = time.perf_counter() - txx
    txx = time.perf_counter()
    print(f'\t\tInitializing bottom column environments ({time.time()-start_time} s)')
    bottom_envs = initialize_bottom_environments(x, y, H, central_col, benvs_tmp, 
                      chi=chi, peps_parity=peps_parity, tmpdir=tmpdir, 
                      profile=profile)
    txx_bote = time.perf_counter() - txx

    print('TtopE = %10.3f TbotE = %10.3f' % (txx_tope, txx_bote))
    if tmpdir is not None:
        os.remove(central_col)
    ...
    for x in range(norm.Lx):
        ...
        # Do the site update
        txx = time.perf_counter()
        E, norm = dmrg_site_update(x, y, H, norm,  
                                   top_envs, bottom_envs, 
                                   peps_parity=peps_parity, 
                                   tmpdir=tmpdir, 
                                   ...
                                   profile=profile,
                                   **site_update_args)
        txx_sup = time.perf_counter() - txx

        # Update the top/bottom environments
        print(f'\t\tMoving Top Environments up ({x},{y}) ({time.time()-start_time} s)')
        top_envs = move_top_environments_up(x, top_envs, tmpdir)
        print(f'\t\tMoving Bottom Environments down ({x},{y}) ({time.time()-start_time} s)')
        if x > 0:
            benvs_tmp = None
        bottom_envs = move_bottom_environments_up(x, y, H, norm, benvs_tmp, bottom_envs, chi=chi, peps_parity=peps_parity, tmpdir=tmpdir, profile=profile)
        print(f'nfile_site_update_{x}_{y}=',len(os.listdir(tmpdir)))

        from pyblock3.algebra.fermion import TIMINGS
        timings = "td = %.3f qr = %.3f svd = %.3f tp = %.3f tot = %.3f" % (*TIMINGS, sum(TIMINGS))
        TIMINGS[0] = TIMINGS[1] = TIMINGS[2] = TIMINGS[3] = 0.0
        print("  Site = %5d %5d E = %20.6f Tsite-up = %10.3f T = %10.3f %s" % (x, y, E, txx_sup, time.perf_counter() - tx, timings))

    # Save the current result
    if save_peps_loc is not None:
        peps = FermionTensorNetwork(norm['KET']).view_like_(peps)
        fname = save_peps_loc+str(time.time())
        print(f'save state to {fname}')
        write_ftn_to_disc(peps, fname, provided_filename=True)

    # Clear the top/bottom environments from disc
    if tmpdir is not None:
        print(f'\t\tRemoving old top environments ({time.time()-start_time} s)')
        for key,removed_env in top_envs.items():
            if key != 'op_order': 
                os.remove(removed_env['tn'])
        print(f'\t\tRemoving old bottom environments ({time.time()-start_time} s)')
        for key,removed_env in bottom_envs.items(): 
            if key != 'op_order': 
                os.remove(removed_env['tn'])

    print(f'nfile_column_sweep_{y}=',len(os.listdir(tmpdir)))
    # Return the result
    return E, norm

def dmrg_sweep_right(H, peps, compress_row=False, chi=-1, benvs=None, peps_parity=None, tmpdir=None, save_peps_loc=None, iprint=0, profile=False, **sweep_args):
    """
    ...
    params = load_dynamical_parameters(tmpdir)
    chi = params['chi']
    chi_row = chi if compress_row else -1

    # Compute the initial boundary environments
    if benvs is None:
        print(f'\tInitializing Boundary Environments with chi={chi}')
        norm, benvs = initialize_boundary_environments(0, H, norm, chi=chi, 
                          peps_parity=peps_parity, tmpdir=tmpdir, profile=profile)
    ...
    for y in range(peps.Ly):
        ...
        E, norm = dmrg_column_sweep(y, H, norm, benvs, peps, chi=chi_row, 
                      peps_parity=peps_parity, tmpdir=tmpdir, 
                      save_peps_loc=save_peps_loc, profile=profile, **sweep_args)

        # Update environments
        if y != peps.Ly-1:
            benvs = move_benvs_right(y, H, norm, benvs, chi=chi, 
                        peps_parity=peps_parity, tmpdir=tmpdir, profile=profile)
...
def dmrg_sweep_left(H, peps, compress_row=False, chi=-1, benvs=None, peps_parity=None, tmpdir=None, save_peps_loc=None, iprint=0, profile=False, **sweep_args):
    """
    ...
    if benvs is None:
        print(f'\tInitializing Boundary Environments with chi={chi}')
        norm, benvs = initialize_boundary_environments(peps.Ly-1, H, norm, chi=chi, 
                          peps_parity=peps_parity, tmpdir=tmpdir, profile=profile)

    # Loop through all columns
    for y in reversed(range(peps.Ly)):
        print(f'\t\tDoing left sweep over rows in column {y}')
        ...
        # Update environments
        if y != 0:
            benvs = move_benvs_left(y, H, norm, benvs, chi=chi, 
                        peps_parity=peps_parity, tmpdir=tmpdir, profile=profile)
...
def dmrg(H, peps, 
    ...
    save_dynamical_parameters(tmpdir, chi=chi, **sweep_args)
    ...
    for itercnt in range(maxiter):
        ...
        E, peps, benvs = dmrg_sweep_right(H, peps, compress_row=compress_row, 
                                          chi=chi, 
                                          benvs=benvs, 
                                          peps_parity=peps_parity, 
                                          tmpdir=tmpdir,
                                          start_time=start_time, iprint=0,
                                          profile = profile,
                                          **sweep_args)
        print("Sweep RT %5d / %5d E = %20.10f T = %10.3f" % (itercnt * 2, maxiter * 2,
            E, time.perf_counter() - tx))
        print(f'nfile_sweep_right_{itercnt}=',len(os.listdir(tmpdir)))

        tx = time.perf_counter()

        # Do left sweep optimization
        E, peps, benvs = dmrg_sweep_left(H, peps, compress_row=compress_row, 
                                         chi=chi, 
                                         benvs=benvs, 
                                         peps_parity=peps_parity,
                                         tmpdir=tmpdir,
        ...
        print("Sweep LF %5d / %5d E = %20.10f T = %10.3f" % (itercnt * 2 + 1, maxiter * 2,
            E, time.perf_counter() - tx))
        print(f'nfile_sweep_left_{itercnt}=',len(os.listdir(tmpdir)))

...
def SVDbackward(dU,dS,dVh,U,S,Vh):
    ...
    if M>=N:
        #dA = torch.mm(U, torch.mm(Su + Sv + torch.diag(dS), Vh))
        dA = np.dot(U, np.dot(Su + Sv + np.diag(dS), Vh))
    else:
        dA = np.dot(np.dot(U, Su + Sv + np.diag(dS)), Vh)
    ...
    if (M>NS):
        ...
        #tmp2 = U.t() @ tmp1
        #tmp2 = U @ tmp2
        #tmp1 = torch.mm(dU*Sinv, Vh)
        #tmp2 = torch.mm(U.t(), tmp1)
        #tmp2 = torch.mm(U,  tmp2)
        tmp1 = np.dot(dU*Sinv, Vh)
        tmp2 = np.dot(U.T, tmp1)
        tmp2 = np.dot(U,  tmp2)
        dA += (tmp1 - tmp2)
    if (N>NS):
        ...
        #tmp2 = tmp1 @ Vh.t()
        #tmp2 = tmp2 @ Vh
        #tmp1 = torch.mm(U*Sinv, dVh)
        #tmp2 = torch.mm(tmp1, Vh.t())
        #tmp2 = torch.mm(tmp2, Vh)
        tmp1 = np.dot(U*Sinv, dVh)
        tmp2 = np.dot(tmp1, Vh.T)
        tmp2 = np.dot(tmp2, Vh)
        dA += (tmp1 - tmp2)
    #if not torch.all(torch.isfinite(dA)):
    #    raise ValueError("dA is not finite")

...
class TNVMC: # stochastic sampling
    ...
    def _ctr(self,samplesize=None):
        ...
        while self.terminate[0]==0:
            ...
            err1_mean += self.buf[3]
            err1_max = max(err1_max,self.buf[3])
            err2_mean += self.buf[4]
            err2_max = max(err2_max,self.buf[4])
            ncurr += 1
            #print(ncurr)
        ...
        print('\tcontraction err single=',err1_mean / len(self.e),err1_max)
        print('\tcontraction err double=',err2_mean / len(self.e),err2_max)
